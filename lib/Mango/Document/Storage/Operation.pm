# ABSTRACT: Standard Mango Document/Collection Operations

use strict;
use warnings;

package Mango::Document::Storage::Operation;
{
    $Mango::Document::Storage::Operation::VERSION = '0.001_01';
}
...
use MongoDB::OID;
use Mango::Document::Storage::Criterion;
use Mango::Document::Child;
use Mango::Document::Relative;
use Mango::Document::GridFile;
...
sub collapse {
    ...
    foreach my $field (keys %{$self->_dirty}) {
    ...
    my $field_config = $self->config->fields;
    for my $name (keys %$field_config) {
        my $config = $field_config->{$name};

        # collapse embedded gridfs documents
        if ($config->{isa} eq 'Mango::Document::GridFile') {
        ...
        elsif ($config->{isa} eq 'Mango::Document::Child') {
        ...
        elsif ($config->{isa} eq 'Mango::Document::Relative') {
...
sub distinct {
    ...
    my ($class, $key, %where) = @_;

    $class = ref $class if ref $class;

    my $search = $class->search->where(%where);

    my $db_name = $class->config->database->{'name'};

    my $col_name = $class->config->collection->{'name'};

    my $db = $class->connection->get_database($db_name);

    my $output = $db->run_command(
        [

            distinct => $col_name,
            key      => $key,
            query    => $search->criteria->{where}

        ]
    );

    return ref $output ? $output->{values} : [];
...
sub execute_on {
    ...
    die
      "execute_on requires a CODEREF to be executed against the target server"
...
sub expand {
    ...
    my $field_config = $class->config->fields;
    for my $name (keys %$field_config) {
        my $config = $field_config->{$name};
        ...
        if ($config->{isa} eq 'Mango::Document::GridFile') {
            ...
            if ($config->{type} eq 'single') {
                ...
                my $gridfile      = Mango::Document::GridFile->new(
                    parent => $class,
                    target => $grid_fs,
            ...
            if ($config->{type} eq 'single') {
                ...
                my $child = Mango::Document::Child->new(
                    parent => $class,
                    target => $config->{class},
                    config => {%child_args},
                );
            ...
            if ($config->{type} eq 'single') {
                ...
                my $relative      = Mango::Document::Relative->new(
                    parent => $class,
                    target => $config->{class},
...
sub insert {
    ...
    my $new_id = $collection->insert($self->collapse, $self->config->options);
    $self->_id($new_id) unless $self->_id;
...
sub reload {
    ...
    #FIXME What is the correct behaviour here, aren't we just always
...
sub search {
    ...
    my $criteria = Mango::Document::Storage::Criterion->new(
        collection => $config->_mongo_collection,
        build_class => ref($self) ? ref($self) : $self,
    );
...
Mango::Document::Storage::Operation - Standard Mango Document/Collection Operations
...
Mango::Document::Storage::Operation is a role that provides
collection-specific functionality to your Mango document classes.
...
The distinct method, called on a class or instance, allows you to quickly query
a column in the collection and return the unique values as an arrayref using the
syntax described in L<Mango::Document::Storage::Criterion>.

The distinct method always returns an arrayref it accepts a single argument
(just the key/column name) and optionally a list of key/value pairs in (query
syntax form).

    my $artists = CDDB::Album->distinct('artist');
    
    my $new_artists = CDDB::Album->distinct('artist', @query);
...
this method has many applications.
    ...
    # or ...
    # if using Mango::Application with connection pooling
...
resultset using the syntax described in L<Mango::Document::Storage::Criterion>.
...
a resultset using the L<Mango::Document::Storage::Criterion> syntax.
...
a resultset using the L<Mango::Document::Storage::Criterion> syntax returning
...
L<Mango::Document::Storage::Criterion> package executing pre-defined filters
...
L<Mango::Document::Storage::Criterion> object allowing you to build complex
queries.
